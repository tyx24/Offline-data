...
object Data_cleansing_4 {
  def main(args: Array[String]): Unit = {
    ...
    //获取ods数据
    val ods = sparkSession.sql("select * from ods.base_region where etl_date=20240401")
    ...
    //获取dwd数据
    val dwd = sparkSession.sql("select * from dwd.dim_region where etl_date=(select max(etl_date) from dwd.dim_region)")
      .drop("etl_date")
    ...
    sparkSession.sql("insert overwrite table dwd.dim_region partition(etl_date='20240401') select * from result_v")
