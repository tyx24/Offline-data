...
object Data_cleansing_1 {
  def main(args: Array[String]): Unit = {
    ...
    val ods_Table_URL = "hdfs://bigdata1:8020/user/hive/warehouse/ods_ds_hudi.db/user_info"

    val dwd_Table_URL = "hdfs://bigdata1:8020/user/hive/warehouse/dwd_ds_hudi.db/dim_user_info"
    ...
    sparkSession.sql(
      """
        |select *,
        | row_number() over (partition by id order by operate_time desc) as `operate_time_num`,
        | min(dwd_insert_time) over (partition by id) as `min_insert_time`
        |from v
        |""".stripMargin)
      .withColumn("dwd_insert_time", when(col("operate_time_num") === 1, col("min_insert_time"))
        .otherwise(col("dwd_insert_time")))
      .filter(col("operate_time_num") === 1)
      .drop("operate_time_num", "min_insert_time")
