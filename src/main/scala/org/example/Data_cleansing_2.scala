...
object Data_cleansing_2 {
  def main(args: Array[String]): Unit = {
    ...
    val Conf = new SparkConf().setMaster("local[*]").setAppName("Data_cleansing_2")
    ...
    //获取ods最新的数据
    val ods = sparkSession.sql("select * from ods.sku_info where etl_date='20240401'")
    ...
    //获取dwd数据
    val dwd = sparkSession.sql("select * from dwd.dim_sku_info where etl_date=(select max(etl_date) from dwd.dim_sku_info)").drop("etl_date")
    ...
    sparkSession.sql(
      """
        |from v
        |select *,
        | row_number() over (partition by v.id order by v.create_time desc) as operate_time_num,
    ...
    sparkSession.sql("insert overwrite table dwd.dim_sku_info partition(etl_date=20240401) select * from result_v")
